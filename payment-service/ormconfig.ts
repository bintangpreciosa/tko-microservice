// D:\tko-microservice\payment-service\ormconfig.ts
...
const configOptions: DataSourceOptions = { // Ubah nama variabel untuk konfigurasinya
  ...
  port: parseInt(process.env.DB_PORT || '3310', 10), // <-- UBAH KE PORT YANG DIEXPOSE DOCKER KE HOST (3310 untuk Payment)
  username: process.env.DB_USERNAME || 'root',
  password: process.env.DB_PASSWORD || '',
  // === PENYESUAIAN KHUSUS UNTUK PAYMENT SERVICE ===
  database: process.env.DB_DATABASE || 'payment_service', // Nama database untuk Payment Service
  
  // Path ke entitas Payment Service.
  entities: [__dirname + '/dist/payment/entity/*.entity.{js,ts}'], 
  
  // Path ke file migrasi Payment Service.
