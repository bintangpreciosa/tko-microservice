    ...
    import { Resolver, Query, Mutation, Args, ID, ResolveField, Parent, ResolveReference, Int, Float } from '@nestjs/graphql'; // Pastikan Int dan Float diimpor
    ...
    import { BadRequestException, NotFoundException } from '@nestjs/common'; // <-- PASTIKAN NotFoundException DIIMPOR
    ...
    export class CartResolver {
      ...
      async cart(@Args('input') input: GetCartInput): Promise<CartDTO | null> {
        if (!input.customer_crm_id && !input.session_id && !input.cart_id) { // Tambahkan validasi cart_id
            throw new BadRequestException('Either cart_id, customer_crm_id or session_id must be provided to get a cart.');
        }
      ...
      async clearCart(@Args('input') input: GetCartInput): Promise<boolean> {
        if (!input.customer_crm_id && !input.session_id && !input.cart_id) { // Tambahkan validasi cart_id
            throw new BadRequestException('Either cart_id, customer_crm_id or session_id must be provided to clear cart.');
        }
      ...
      // @ResolveReference untuk CartDTO
      @ResolveReference()
      ...
      async getCustomer(@Parent() cart: CartDTO): Promise<CustomerReference | null> {
        ...
        return { id: cart.customer_crm_id }; // Tambahkan __typename
      }

      // Field Resolver untuk Product di dalam CartItemDTO (yang sudah ada)
      // Ini adalah Field Resolver untuk OrderItemDTO.product, bukan CartDTO.product
      @ResolveField('product', () => ProductReference) // <-- PERHATIKAN: INI UNTUK OrderItemDTO.product
      async getProduct(@Parent() cartItem: CartItemDTO): Promise<ProductReference> {
        // Hapus `if (!cartItem.product_id) throw ...` karena DB sudah menjamin NOT NULL
        return { product_id: cartItem.product_id }; // Tambahkan __typename
      }

      // Field Resolver untuk productDetailSummary di CartItemDTO
      @ResolveField(() => String, { nullable: true })
      productDetailSummary(@Parent() cartItem: CartItemDTO): string | null {
        // Pastikan Anda mengakses ProductReference dari cartItem.product
        const { name, price, stock } = cartItem.product; // Ini product dari ProductReference
